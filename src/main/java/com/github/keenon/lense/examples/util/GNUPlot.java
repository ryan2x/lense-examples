package com.github.keenon.lense.examples.util;
...
public class GNUPlot {
    ...
    private List<String> lineTitle = new ArrayList<>();
    ...
    public void addLine(String title, double[] xData, double[] yData) {
    ...
    private void dumpScript(String folder) throws IOException {
        ...
        for (int i = 0; i < xAxisData.size(); i++) {
            bw.append("'").append(folder).append("/").append(lineTitle.get(i)).append(".dat' with lines title '"+lineTitle.get(i)+"'");
    ...
    private void dumpDatFiles(String folder) throws IOException {
        ...
        for (int i = 0; i < xAxisData.size(); i++) {
            BufferedWriter bw = new BufferedWriter(new FileWriter(folder+"/"+lineTitle.get(i)+".dat"));
