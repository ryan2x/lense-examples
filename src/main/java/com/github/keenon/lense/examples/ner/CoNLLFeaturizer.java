...
public class CoNLLFeaturizer {
    ...
    public static void annotate(GraphicalModel model, String[] tags, ConcatVectorNamespace namespace) {
        if (embeddings == null) {
            ObjectInputStream ois = null;
            try {
                ois = new ObjectInputStream(new GZIPInputStream(new FileInputStream("lense-examples/src/main/resources/ner/google-300-trimmed.ser.gz")));
        ...
        for (int i = 0; i < model.variableMetaData.size(); i++) {
            ...
            if (i < model.variableMetaData.size() - 1) {
                if (model.getVariableMetaDataByReference(i+1).get("TOKEN") != null) {
                    GraphicalModel.Factor jf = model.addFactor(new int[]{i, i + 1}, new int[]{tags.length, tags.length}, (assignment) -> {

                        // This is the anonymous function that generates a feature vector for every joint assignment to the
                        // binary factor

                        String thisTag = tags[assignment[0]];
                        String nextTag = tags[assignment[1]];
